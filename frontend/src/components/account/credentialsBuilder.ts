...
export function isHeaderOverrideCapable(
  ...
  if (
    ...
    return type === "apikey" || (platform === "openai" && type === "relay");
  }
  if (platform === "grok") {
...
export function validateHeaderOverrideRows(
  ...
  for (const row of rows) {
    ...
    if (
      !isValidHeaderOverrideName(name) ||
      name.length > HEADER_OVERRIDE_MAX_NAME_LENGTH
    ) {
      return "invalidName";
    }
 ...
 */
export function parseHeaderOverridesJson(
  text: string,
): HeaderOverrideRow[] | null {
  let parsed: unknown;
  try {
...
export const CN_BASE_URL_PRESETS: Record<
  "kimi" | "zhipu" | "deepseek",
  CnBaseUrlPreset[]
> = {
  kimi: [
...
export function cnQuotaCellVisible(
  platform: string,
  accountMode: string,
): boolean {
  return (
    (platform === "kimi" || platform === "zhipu") && accountMode === "coding"
  );
}

export function cnBalanceCellVisible(
  platform: string,
  accountMode: string,
): boolean {
  return (
    (platform === "kimi" || platform === "deepseek") && accountMode !== "coding"
  );
}
...
export function applyHeaderOverride(
  ...
  if (enabled) {
    ...
    credentials[HEADER_OVERRIDES_CREDENTIAL_KEY] =
      buildHeaderOverridesObject(rows);
  } else if (mode === "edit") {
